package fish.member.flavor.dto;

import fish.domain.flavor.index.BungFlavorEntity;
import lombok.Builder;
import lombok.Getter;

@Getter
public class FlavorResponse {
    private Long id;
    private String flavor;
    private String iconCode;
    private int seq;

    @Builder
    public FlavorResponse(Long id, String flavor, String iconCode, int seq) {
        this.id = id;
        this.flavor = flavor;
        this.iconCode = iconCode;
        this.seq = seq;
    }

    public static FlavorResponse toResponseDTO(BungFlavorEntity bungFlavorEntity) {
        return FlavorResponse.builder()
                .id(bungFlavorEntity.getId())
                .flavor(bungFlavorEntity.getFlavor())
                .iconCode(bungFlavorEntity.getIconCode())
                .seq(bungFlavorEntity.getSeq())
                .build();
    }
}
