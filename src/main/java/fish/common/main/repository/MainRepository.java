...
public interface MainRepository extends JpaRepository<DetailEntity, Long> {
    @Query(value = "SELECT COUNT(*) AS weeklyCount, GROUP_CONCAT(DAYNAME(`regDate`)) AS days " +
                    "FROM FISH_BUN_DETAIL " +
                    "WHERE userId = ?1 AND `regDate` BETWEEN " +
    ...
    @Query(value = "SELECT COUNT(*) " +
                    "FROM FISH_BUN_DETAIL " +
                    "WHERE userID =?1 AND MONTH(`regDate`) = MONTH(CURDATE()) AND YEAR(`regDate`) = YEAR(CURDATE())", nativeQuery = true)
