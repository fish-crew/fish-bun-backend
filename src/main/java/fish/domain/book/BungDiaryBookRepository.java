...
@Repository
public interface BungDiaryBookRepository extends JpaRepository<BungDiaryBook, Long> {
    List<BungDiaryBook> findAllByUserId(Long userId);
    ...
    @Query(value = """
        ...
        FROM BUNG_FLAVOR FBF
        LEFT JOIN BUNG_DIARY_BOOK UFBB
        ON FBF.id = UFBB.completedFlavorId AND UFBB.userId = :userId
        LEFT JOIN BUNG_DIARY_BOOK UFBB_ALL
    ...
    Optional<BungDiaryBook> findByUserIdAndCompletedFlavorId(Long userId, Long flavorId);
}